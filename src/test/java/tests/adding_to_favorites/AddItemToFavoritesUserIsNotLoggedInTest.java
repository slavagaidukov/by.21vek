...
public class AddItemToFavoritesUserIsNotLoggedInTest extends BaseTest {
    ...
    public void test() {
        ...
        verifyTrue(itemPage.isPossibleToRemoveItemFromFavorites(), "Verify if it possible to remove item from favorites");
        ...
        verifyTrue(favoriteItemsPage.itemExists(ITEM_NAME), "Verify item visibility");
        verifyEquals(favoriteItemsPage.getItemPrice(ITEM_NAME), price,
                "Verify item price on Favorite Items page");
        verifyTrue(favoriteItemsPage.isPossibleToRemoveItemFromFavorites(ITEM_NAME), "Verify if it possible to remove item from favorites");
       ...
       verifyTrue(!accountPopup.isFavoriteItemsCountElementVisible(), "Verify favorite items count element visibility");
